...
REST（Representational State Transfer）是一種用於設計網頁應用程式的架構風格。 RESTful API 是基於 REST 原則構建的應用程式接口，旨在透過 HTTP 協定進行資源的存取和操作。

### RESTful API 的基本原則

1. **無狀態性（Stateless）**：每個請求都是獨立的，伺服器不儲存任何關於客戶端的狀態資訊。所有的狀態資訊都應包含在請求中。

2. **資源的表現（Resource Representation）**：資源透過 URI（統一資源標識符）進行標識，客戶端與伺服器之間透過 JSON、XML 或其他格式進行資源的表示。

3. **統一介面（Uniform Interface）**：RESTful API 提供一致的接口，透過 HTTP 方法（GET、POST、PUT、DELETE）對資源進行操作。

4. **可緩存性（Cacheable）**：回應應包含快取訊息，以提高效能並減少伺服器負擔。

5. **分層系統（Layered System）**：系統可以由多層組成，客戶端無法直接知道是否與最終伺服器交互，增強了系統的可擴展性和安全性。
### HTTP 方法

| HTTP 方法 | 描述     | 用途               |
|---------|--------|------------------|
| GET     | 獲取資源   | 用于讀取資源           |
| POST    | 創建資源   | 用於向伺服器提交資料以建立新資源 |
| PUT     | 更新資源   | 用於更新現有資源         |
| DELETE  | 刪除資源   | 用於刪除指定資源         |
| PATCH   | 部分更新資源 | 用於對已有資源進行部分更新    |