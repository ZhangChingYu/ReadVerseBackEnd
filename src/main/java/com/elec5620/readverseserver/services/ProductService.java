...
public class ProductService {
    ...
    @Autowired
    public ProductService(BookRepository bookRepository, UserRepository userRepository) {
    ...
    public FormalDto getProduct(Long publisherId) {
    ...
    public FormalDto editProduct(BookDto bookDto) {
        FormalDto result = FormalDto.builder().build();
    ...
    public FormalDto deleteProduct(Long bookId, Long publisherId) {
        FormalDto result = checkPublisher(publisherId);
        if (result != null) return result;
        ...
        if (!book.get().getPublisherId().equals(publisherId)) {
            result.setStatus(403);
            result.setMessage("User not allow to delete");
            return result;
        }

        bookRepository.delete(book.get());
        FileHandler.deleteFile(publisherId, bookId);
        result.setStatus(200);
    ...
    public FormalDto searchProduct(Long publisherId, String keyword) {
        FormalDto result = checkPublisher(publisherId);
        if (result != null) return result;
        ...
        if (!keyword.isEmpty()) {
            ...
            List<Book> books = bookRepository.findBooksByPublisherId(publisherId);
            result.setMessage("Return all books");
            result.setData(books);
        }
        return result;
    }
